...
contract ethereumAds {
    ...
    function buyOwnedBox(uint _box_index) external payable {
        ...
        require(adBoxes[_box_index].rented == false , "This box is currently rented. Wait until the renting period ends to purchase it.");
    ...
    function rentBox(uint _box_index, uint _many_days) external payable {
        ...
        require(adBoxes[_box_index].rented == false , "The box is already on rent.");
        require(adBoxes[_box_index].rent_price_daily != 0 , "The box is not for rent.");
         
        adBoxes[_box_index].rent_start = block.timestamp;
    ...
    function retrieveRentedBox(uint _box_index) external {
        ...
        require(adBoxes[_box_index].rent_start + adBoxes[_box_index].rent_period >= block.timestamp, "Rent period didn't end yet.");
    ...
    function getPrice(uint _box_index) external view returns (uint) {
        return adBoxes[_box_index].buy_price;
    }
